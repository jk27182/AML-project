...
class NAMDataset(CSVDataset):
    ...
    def __init__(self,
        ...
        self.setup_dataloaders(val_split=config.val_size, test_split=config.test_size)
    ...
    def setup_dataloaders(self, val_split: float = 0.1, test_split: float = 0.2) -> Tuple[DataLoader, ...]:
        ...
        # hier muss ich ein resampling fuer train machen mit weighted random resampler
        y_train = torch.cat([batch[1] for batch in train_subset], dim=0)
        weights = np.where(y_train == 0, 1/len(y_train[y_train==0]), 1/len(y_train[y_train==1]))
        weighted_sampler = WeightedRandomSampler(weights=weights, num_samples=len(y_train))

        self.train_dl = DataLoader(train_subset, batch_size=self.config.batch_size, sampler=weighted_sampler)
