...
from nam.trainer.metrics import auroc
...
class LitNAM(pl.LightningModule):
    ...
    def __init__(self, config: Config, model: nn.Module) -> None:
        ...
        self.metrics = lambda logits, targets: mae(logits, targets) if config.regression else auroc(logits, targets)
        self.metrics_name = "MAE" if config.regression else "AUROC"
